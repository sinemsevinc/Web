...
$selectProduct = mysqli_query($con, "SELECT * FROM jeuvideo WHERE JeuID = '$productId'");
...
<div class="cont">
    ...
    if (isset($data["add-to-cart"])){
        $id2 = $data["id2"];
        $quantity = $fetchProduct["quantite"];
        mysqli_query($con, "UPDATE jeuvideo SET quantite = '$quantity' - 1 WHERE JeuID = '$id2'");
    }
?>
        </div>
</div>